...
switch($_GET[act]){
	...
	if(isset($_POST['Go'])){
			...
			$sql = mysqli_query($conn, "SELECT tsiswa.nis, tsiswa.nama, tabsen.keterangan, tabsen.id_absen FROM tsiswa LEFT JOIN tabsen ON tsiswa.nis = tabsen.nis AND tanggal='$_POST[tanggal]' AND id_semester = '$semester[id_semester]' AND tsiswa.nama = '$namasiswa'");
			$no = 1;
	...
	$namasiswa = $_SESSION[nama_lengkap];
	$sql = mysqli_query($conn, "SELECT * FROM tsiswa where nama = '$namasiswa'");
	$dataread = mysqli_fetch_assoc($sql);

	echo "<h3>Tambah Absen</h3>";
	echo "	<body style='font-size:65%;'>
			<form method=POST action=$aksi?module=absen&act=input>
			<table border=1>
				<tr><td><font face=tahoma size=2>NIS</font></td><td><font face=tahoma size=2>:</font></td>
				<td><input type='text' name='nis' size=32 value='$dataread[nis]' readonly></td></tr>
				<tr><td><font face=tahoma size=2>Tanggal</font></td>
				<td><font face=tahoma size=2>:</font></td><td>
				<input id='tanggal' type='text' name='tanggal' size=32 required> </td></tr>
				...
				<input type='radio' name='keterangan' value='A' required> Alpha 
				</font></td></tr>
	...
	echo "	<body style='font-size:65%;'><form method=POST action=$aksi?module=absen&act=update>
			...
			<table border=1>
				<tr><td><font face=tahoma size=2>NIS</font></td><td><font face=tahoma size=2>:</font></td>
				<td><input type='text' name='nis' size=32 value='$data[nis]' readonly></td></tr>
				<tr><td><font face=tahoma size=2>Nama</font></td>
				<td><font face=tahoma size=2>:</font></td>
				<td><input type='text' name='nama' size=32 value='$data[nama]' readonly></td></tr>
				<tr><td><font face=tahoma size=2>Tanggal</font></td>
				<td><font face=tahoma size=2>:</font></td>
				<td><input id='tanggal' type='text' name='tanggal' size=32 value='$data[tanggal]'> </td></tr>
				<tr><td><font face=tahoma size=2>Keterangan</font></td>
				<td><font face=tahoma size=2>:</font></td>
