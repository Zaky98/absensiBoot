...
$aksi = "modul/mod_usersiswa/aksi_user.php";
switch($_GET[act]){
	default:
	...
	$sql = mysqli_query($conn, "SELECT * FROM tusersiswa ORDER BY username ASC");
	$no =1;
	...
	?>
		<tr>
			...
			<td><font face=tahoma size=2><a href='?module=usersiswa&act=updateuser&id=<?php echo $data[id_user]; ?>'><img src='../images/update.png' title='Update <?php echo $data[username]; ?>'></a> | <a href="<?php echo $aksi; ?>?module=usersiswa&act=hapus&id=<?php echo $data[id_user]; ?>" onclick="return confirm('Anda yakin ingin menghapus User <?php echo $data[username]; ?>?');"><img src='../images/hapus.png' title='Hapus <?php echo $data[username]; ?>'> </a></font></td>
		</tr>
	<?php
		$no++;		
	}
	...
	echo "<input type=button value='Tambah User' onclick=\"window.location.href='?module=usersiswa&act=tambahuser';\" class='btn-primary'>";
	echo "<br>";
	echo "   ‏‏‎ ";
	...
	echo "<form method=POST action=$aksi?module=usersiswa&act=input>
			<table border=1>
				<tr><td><font face=tahoma size=2>Nama Pengguna</font></td>
				<td><font face=tahoma size=2>:</font></td><td>
				<input type='text' name='nama_lengkap' required></td></tr>
	...
	case "updateuser":
	$data = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM tusersiswa WHERE id_user = '$_GET[id]'"));
	echo "<h3>Ubah User</h3>";
	echo "<form method=POST action=$aksi?module=usersiswa&act=update>
